"""
Phase 0 -- boucle d'entrainement complete pour ToyLLM sur les .bin produits par prepare_data.py.

Exemple (config toy par defaut, ~28M params) :
    ...
    for p in (train_path, val_path):
        if not os.path.exists(p):
            raise FileNotFoundError(
                f"{p} introuvable. Lance d'abord : python prepare_data.py --out_dir {data_dir}"
            )
...
@torch.no_grad()
def print_sample(model, enc, device, max_new_tokens, prompt="Once upon a time,"):
    model.eval()
